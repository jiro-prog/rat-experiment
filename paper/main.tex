...
\begin{algorithm}[t]
\caption{Relative Anchor Translation}
\label{alg:rat}
\begin{algorithmic}[1]
\REQUIRE Query $x$ via model $X$; database $\{y_1,\dots,y_n\}$ via model $Y$; candidate anchors $\mathcal{C}$
\STATE \textbf{Offline (once per model pair):}
\STATE \quad Encode candidates: $E_X = f_X(\mathcal{C})$
\STATE \quad Select $K$ anchors via FPS on $E_X$: $\mathcal{A} = \mathrm{FPS}(E_X, K)$
\STATE \quad Encode anchors: $A_X = f_X(\mathcal{A})$, $A_Y = f_Y(\mathcal{A})$
\STATE \quad Compute DB relative repr.: $R_Y[j,k] = (f_Y(y_j)^\top A_Y[k] + 1)^2$
\STATE \quad Apply z-score to $R_Y$ (row-wise): $\hat{R}_Y$
\STATE \textbf{Online (per query):}
\STATE \quad $r_X(x) = [(f_X(x)^\top A_X[k] + 1)^2]_{k=1}^K$
\STATE \quad Retrieve: $\hat{j} = \arg\max_j \cos(r_X(x),\, \hat{R}_Y[j])$
\end{algorithmic}
\end{algorithm}
...
For cross-modal retrieval between a text encoder $f_T$ and an image encoder $f_I$, we introduce \textbf{Rosetta Stone anchors}: $K$ concept-aligned pairs $\{(t_k, i_k)\}_{k=1}^K$ where $t_k$ is a caption and $i_k$ the corresponding image (Figure~\ref{fig:pipeline}b). Each model encodes its own modality:
\begin{align}
...
\end{align}
The key insight is that both representations encode the same concept's \emph{response pattern}, measured through different modalities. We use image-caption pairs from the COCO Karpathy test split \citep{lin2014microsoft,karpathy2015deep}, with FPS applied in the text encoder's space.

\begin{figure}[t]
  \centering
  \includegraphics[width=\textwidth]{figures/figure1_rat_pipeline.png}
...
\begin{table}[t]
\centering
\caption{Recall@1 (\%) vs.\ anchor count $K$ on A$\times$B (STSBenchmark).}
\label{tab:scaling}
\begin{tabular}{@{}rccc@{}}
\toprule
$K$ & Random + cosine & RAT protocol & $\Delta$ \\
\midrule
100  & 26.0 & \textbf{55.0} & +29.0 \\
200  & 33.4 & \textbf{69.8} & +36.4 \\
500  & 45.2 & \textbf{79.6} & +34.4 \\
1000 & 48.2 & \textbf{79.6} & +31.4 \\
\bottomrule
\end{tabular}
\end{table}

Table~\ref{tab:scaling} and Figure~\ref{fig:scaling} compare the na\"ive baseline against the full RAT protocol. \textbf{RAT at $K{=}100$ (55.0\%) surpasses random at $K{=}1000$ (48.2\%)}---proper anchor selection and kernel choice achieve higher accuracy with 10$\times$ fewer anchors. RAT saturates at $K{=}500$, identifying it as the cost-optimal operating point.

\begin{figure}[t]
  \centering
  \includegraphics[width=0.85\textwidth]{figures/scaling_comparison.png}
  \caption{Scaling curves for A$\times$B: Random+cosine (dashed) vs.\ RAT protocol (solid).}
  \label{fig:scaling}
\end{figure}
...
\begin{table}[t]
\centering
\caption{Cross-modal retrieval between MiniLM text and CLIP ViT-B/32 image.}
\label{tab:crossmodal}
\begin{tabular}{@{}llccc@{}}
\toprule
Direction & Method & Recall@1 & Recall@10 & MRR \\
\midrule
Text $\to$ Image & baseline      & \textbf{16.4} & 62.6 & 0.305 \\
Text $\to$ Image & z-score       & 15.6 & 60.4 & 0.297 \\
Image $\to$ Text & baseline      & 3.8  & 23.8 & 0.110 \\
Image $\to$ Text & z-score       & \textbf{18.2} & 64.6 & 0.325 \\
\midrule
\emph{CLIP direct} & \emph{upper bound} & \emph{62.0} & \emph{95.0} & \emph{0.734} \\
\bottomrule
\end{tabular}
\end{table}

Table~\ref{tab:crossmodal} shows that a text-only encoder achieves Recall@1 = 18.2\% in a vision encoder's space---91$\times$ the random baseline---using 500 Rosetta Stone anchors and zero training. RAT achieves 29\% of CLIP's retrieval accuracy (18.2/62.0), where CLIP was trained on 400M image-text pairs.
...
\begin{figure}[t]
  \centering
  \includegraphics[width=0.85\textwidth]{figures/analysis_sim_dist.png}
...
Models B (E5-large, 1024d) and E (BGE-large, 1024d) share output dimensionality but differ in architecture and training. Their direct cosine similarity on identical texts averages $-0.02$---indistinguishable from random projections. Yet RAT achieves 55.2\% on B$\times$E. Meanwhile, A$\times$B (384d $\times$ 1024d) achieves 83.8\%.

\textbf{Same-dimension B$\times$E (55\%) $<$ cross-dimension A$\times$B (84\%)}: dimensional agreement is irrelevant to RAT performance. What matters is the structural compatibility of the underlying semantic spaces. RAT's Recall@1 provides a concrete operationalization of the structural convergence posited by the Platonic Representation Hypothesis \citep{huh2024platonic}: independently trained models develop partially overlapping semantic geometries visible only through relative comparison. The spectrum of RAT scores (55\%--98\%) offers a finer-grained measure than a binary ``converged or not'' framing.
...
RAT is a zero-shot protocol for translating between embedding spaces without training. Three composable steps---FPS anchor selection, polynomial kernel similarity, and DB-side z-score normalization---enable cross-model retrieval across five encoders with Recall@1 from 55\% to 98\%.

Our experiments yield three findings that extend beyond RAT: (1)~Similarity collapse is a diagnosable, fixable failure mode of relative representations, resolved by z-score normalization (+65 points) while leaving well-spread models unharmed. (2)~z-score is a DB-side operation: query-side normalization is unnecessary and harmful. (3)~Relative structure persists when absolute coordinates are uncorrelated (cos $\approx 0$, RAT 55\%), providing an operational measure of the structural convergence posited by the Platonic Representation Hypothesis.

Rosetta Stone anchors extend RAT across modalities: a text-only encoder retrieves from a vision encoder's space at 18.2\% Recall@1 with zero visual training. RAT's appeal lies in simplicity: adding a new model requires only encoding shared anchors---no paired data, no gradients, no GPU.
