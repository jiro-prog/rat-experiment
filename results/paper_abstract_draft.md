...
We evaluate RAT across five text embedding models (384d–1024d, spanning BERT, XLM-R, and CLIP architectures) on two datasets (STSBenchmark, AllNLI). All model pairs achieve Recall@1 between 55% and 98%, compared to a 0.2% random baseline. RAT with 100 anchors (55%) surpasses random anchor selection with 1,000 (48%), demonstrating 10× efficiency from proper anchor placement.

Analysis reveals three findings: (1) **Similarity collapse** — a failure mode where compressed similarity distributions flatten relative representations — is diagnosable via entropy and fully resolved by z-score normalization (+50 points). (2) z-score normalization is effective **only on the database side**; query-side normalization is consistently harmful. (3) Two models with identical dimensions (1024d) but zero direct cosine correlation achieve 55% retrieval via RAT, demonstrating shared relative structure invisible to absolute comparison.
