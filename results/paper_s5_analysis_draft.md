...
This finding resonates with the Platonic Representation Hypothesis (Huh et al., 2024), which posits that models trained on sufficient data converge toward a shared statistical model of reality. RAT provides a concrete, quantitative lens for measuring this convergence: the Recall@1 between two models' relative representations can be interpreted as an operational measure of their structural alignment. B×E's 55% — high enough to be useful, low enough to indicate meaningful differences — suggests that convergence is real but partial, and varies with training conditions.
