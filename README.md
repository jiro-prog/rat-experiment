...
全ペアでRecall@1 > 60%を達成。ランダムベースライン(0.2%)の300倍以上。
...
- テキスト×テキストは英語（STSBenchmark）のみ検証。多言語・クロスリンガルは未検証
...
- 非対称z-score（方向ごとにz-score適用を切り替える）は未検証だが、改善余地がある
